...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([dcc.Graph(id='bar-cat', config={'displayModeBar': False})], style={'width': '48%', 'display': 'inline-block'}),
        html.Div([dcc.Graph(id='bar-corr', config={'displayModeBar': False})], style={'width': '48%', 'display': 'inline-block'})
    ]),
...
def update_graphs(target, cat_col):
    try:
        df = df_global.copy()
        if target is None or cat_col is None or df.empty or target not in df.columns or cat_col not in df.columns:
            return {}, {}
...
def train_model(n, target, features):
    ...
    try:
        ...
        return f"✅ Model trained. R2 score: {r2:.2f}"
    except Exception as e:
...
def make_prediction(n_clicks, input_str):
    ...
    try:
        ...
        return f"📊 Predicted {target_column}: {pred[0]:.2f}"
    except Exception as e:
